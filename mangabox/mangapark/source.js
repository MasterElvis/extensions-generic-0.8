...
const MangaBoxHelpers_1 = require("../MangaBoxHelpers");
const SITE_DOMAIN = 'https://mangapark.com';
...
class Mangapark extends MangaBox_1.MangaBox {
    constructor() {
        super(...arguments);
        // Website base URL.
    ...
    async getSearchResults(query, metadata) {
        ...
        if (query.includedTags && query.includedTags?.length != 0) {
            ...
            for (const manga of $('div.panel_story_list div.story_item').toArray()) {
                ...
                const title = (0, entities_1.decodeHTML)($('h3.story_name a', manga).first().text().trim() ?? '');
                const subtitle = (0, entities_1.decodeHTML)($('h3.story_name + em.story_chapter a', manga).text().trim() ?? '');
                if (!mangaId || !title || collecedIds.includes(mangaId))
                    continue;
...
},{"../MangaBox":70,"../MangaBoxHelpers":71,"@paperback/types":61,"entities":69}]},{},[74])(74)
});
